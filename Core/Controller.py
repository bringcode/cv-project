...
class Controller:
    ...
    # 10cm 앞으로 이동
    def walk_10cm():
        minus_10cm = Sensor.dist_measure.dist - 10  # 처음 거리에서 10cm 빼기
        # time으로 몇초 이동하는지 확인하기

        if Sensor.dist_measure.dist != minus_10cm:  # dist: 현재 인식되는 거리
            Robo._motion.walk_cm("FORWARD")

    #####################################
    # 30cm 앞으로 이동
    def walk_30cm():
        minus_30cm = Sensor.dist_measure.dist - 30  # 처음 거리에서 30cm 빼기
        # time으로 몇초 이동하는지 확인하기

        if Sensor.dist_measure.dist != minus_30cm:  # dist: 현재 인식되는 거리
    ...
    def walk_to_ball_and_flag(self):
        ...
        # 공 인식
        while True:
            if Sensor.dist_measure.name == 'ball':
                break
            else:  # 공이 안 보이면 찾게끔
                head_turn = 10
        ...
        # 공 쪽까지 걸어가기
        while True:
            if Sensor.dist_measure.dist <= 공과의_목표_거리:
                self.walk()
            else:
                break

    #####################################
    # (공이랑 홀컵이 로봇 시야에 함께 있는 상황) 홀컵 인식하고, 목표 지점으로 삼아 공 치기
    # 깃발, 홀컵 찾기
    def flag_ball_hit():
        while True:
            if Sensor.dist_measure.name == "flag":
                # 공이랑 깃발 일직선이 되게끔 각도 바꾸기
                while True:
                    # 공이랑 깃발이 일직선이 아니면 계속 턴하면서 찾기
                    if (Sensor.dist_measure.name == "ball") == (
                            Sensor.dist_measure.name == "flag"):  # 일직선이 되면 멈추고 공 치기 --> 아니면 ball_center.py에서 일자 되면 치기
                        Motion.motion.hit_the_ball()
                        break
                    else:
                        body_turn = 10
